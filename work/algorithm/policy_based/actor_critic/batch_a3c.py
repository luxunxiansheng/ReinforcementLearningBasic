...
from algorithm.policy_based.actor_critic.actor_critic_common import \
    ValueEstimator
...
class GlobalBatchCritic(CriticBase):
    def __init__(self,global_value_estimator,discount=1.0):
        self.estimator = global_value_estimator
...
class GlobalBatchActor(ActorBase):
    ENTROY_BETA = 0.0
    
    def __init__(self,global_policy,global_policy_estimator,discount=1.0):
        self.policy = global_policy 
        self.discount = discount
        self.estimator = global_policy_estimator
    ...
    def improve(self,*args):
        ...
        total_loss =  total_policy_loss - GlobalBatchActor.ENTROY_BETA*torch.stack(entroys).sum()
...
class BatchA3C:
    ...
    MAX_STEPS = 500000
    NUM_PROCESS = 8
    def  __init__(self,global_critic,global_actor,env,num_episodes):
    ...
    def _run_one_episode(env,local_critic,local_actor):
        ...
        for step in range(0,BatchA3C.MAX_STEPS):
    ...
    @staticmethod
    def _improve(global_critic,global_actor,env,num_episodes):
        local_env = BatchA3C._create_env(env)
        local_value_estimator = LocalValueEestimator(local_env.observation_space.shape[0])
        local_value_estimator.model.load_state_dict(global_critic.estimator.model.state_dict())
        local_critic = LocalBatchCritic(local_value_estimator,global_critic.estimator,global_critic.estimator.optimizer)

        local_policy_estimator = LocalPolicyEsitmator(local_env.observation_space.shape[0],local_env.action_space.n)
        local_policy_estimator.model.load_state_dict(global_actor.policy.estimator.model.state_dict())
        local_policy = ParameterizedPolicy(local_policy_estimator)
        local_actor = LocalBatchActor(local_policy,global_actor.policy,global_actor.policy.estimator.optimizer)
    ...
    def improve(self):
        BatchA3C._improve(self.global_critic,self.global_actor,self.env,self.num_episodes)
    ...
    def improve_(self):
        ...
        for rank in range(0,BatchA3C.NUM_PROCESS):
            p = mp.Process(target=BatchA3C._improve,args=(self.global_critic,self.global_actor,self.env,self.num_episodes))
            p.start()
            processes.append(p)
