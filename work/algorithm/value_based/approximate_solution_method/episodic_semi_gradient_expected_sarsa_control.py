...
class ApproximationExpectedSARSACritic(CriticBase):
    ...
    def set_create_distribution_fn(self,create_distribution_fn):
...
class EpisodicSemiGradientExpectedSarsaControl:
    ...
    def __init__(self,env, estimator,statistics, episodes):
        ...
        policy =       ContinuousStateValueBasedPolicy(estimator)
        self.critic =  ApproximationExpectedSARSACritic(env,estimator,policy) 
        explorer    =  ESoftExplorer(policy,self.critic)
        self.critic.set_create_distribution_fn(explorer.get_create_distribution_fn())
