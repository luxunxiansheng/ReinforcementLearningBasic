...
def policy_evaluate(tabular_implict_policy,env_transition,discrte_env):
    """
    ...
    for state in tqdm(range(discrte_env.nS)):   #  For each state in env
        for action_index_of_current_state in range(len(tabular_implict_policy.Q_table[state])):  # For each action in current state
            current_env_transition= env_transition[state][action_index_of_current_state]           
            value_of_action = _get_value_of_action(current_env_transition, tabular_implict_policy)  #  the reward is also related to the next state
            new_q_table[state][action_index_of_current_state]= value_of_action  
    
    tabular_implict_policy.Q_table = new_q_table  

def _get_value_of_action(current_env_transition, tabular_implict_policy):
    value_of_action = 0 
...
def _get_value_of_state(tabular_implict_policy, state):
    value_of_state = 0 
