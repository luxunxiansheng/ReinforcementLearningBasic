...
class V_Monte_Carlo_Method:
    def __init__(self, v_table, env, episodes=10000, discount=1.0):
    ...
    def evaluate(self, policy):
        for _ in tqdm(range(0, self.episodes)):
            ...
            for state_index, reward in trajectory[::-1]:
                ...
                return_tuple = (
                    self.return_table[state_index][0]+1, self.return_table[state_index][1]+R)
                self.return_table[state_index] = return_tuple
                self.v_table[state_index] = self.return_table[state_index][1] / \
                    self.return_table[state_index][0]
