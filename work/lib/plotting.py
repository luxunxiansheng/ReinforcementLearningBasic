...
EpisodeStats = namedtuple(
    "EpisodeStats", ["algo", "episode_lengths", "episode_rewards"])


def plot_cost_to_go_mountain_car(env, estimator, num_tiles=20):
    def cost_to_go(observation):
        q_values = []
    ...
    X, Y = np.meshgrid(x, y)
    Z = np.apply_along_axis(cost_to_go, 2, np.dstack([X, Y]))
...
def plot_episode_stats(stats, smoothing_window=10, noshow=False):
    ...
    for index in range(len(stats)):
        plt.plot(pd.Series(stats[index].episode_rewards).rolling(
            smoothing_window, min_periods=smoothing_window).mean(),label=stats[index].algo)
    ...
    for index in range(len(stats)):
        plt.plot(np.cumsum(stats[index].episode_lengths),
                 np.arange(len(stats[0].episode_lengths)),label=stats[index].algo)
